<template>
    <div class="page-actions">
        <NuxtLink :to="`/contacts/customers/`">
            <Button
                variant="secondary"
...
const data = defineModel<Partial<Customers>>();
...
const submit = async () => {
    ...
    try {
        ...
        await useClient(`/api/contacts/customers/${uuid.value}`, {
            method: "PUT",
            body: formData,
        });

        fieldsToClear.forEach((field) => {
            if (data.value) data.value[field as keyof Customers] = "" as any;
        });
