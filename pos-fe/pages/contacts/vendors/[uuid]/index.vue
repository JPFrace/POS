...
definePageMeta({
    permission: "Contacts.Vendors.Edit",
});
...
const defaultForm: Partial<Vendors> = {
    id_no: "",
    id_no_auto: true,
    first_name: "",
    last_name: "",
...
const savedData = ref<Partial<Vendors>>({ ...defaultForm });
const form = ref<Partial<Vendors>>({ ...defaultForm });
...
const {
    ...
    `${id(route.fullPath)}.vendors`,
    async () =>
        await useClient(`/api/contacts/vendors/${route.params.uuid}`, {
            method: "GET",
            params: {
                query: {
                    createdBy: true,
