            ...
            >
                ...
                <tbody>
                    <tr v-for="(item, index) in data!.items">
                        ...
                        >
                            {{
                                currencyFormat(
                                    numberOnly(item.quantity) *
                                        numberOnly(item.rate)
                                )
                            }}
                        </td>
                        <td
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="align-middle text-center">
                            {{ currencyFormat(quantities) }}
                        </td>
                        <td class="align-middle text-center">
                            {{ currencyFormat(rates) }}
                        </td>
...
const total = (key: string, items?: OrderItem[]): number =>
...
const subTotal = computed(() =>
    (data.value?.items ?? []).reduce((sum: any, item: any) => {
        sum += numberOnly(item.quantity) * numberOnly(item.rate);
...
const refill = () => {
    for (var i = (data.value?.items ?? []).length - 1; i < 3; i++) {
        data.value!.items = (
            (data.value!.items ?? []) as unknown as OrderItem[]
        ).concat({
            product: null,
            rate: 0,
            quantity: 0,
...
const onChangeAccount = (value: any) => {
    data.value?.items?.map((row: OrderItem) => {
