    ...
    >
        <template #columns>
            ...
            <el-table-column prop="file.url" label="Photo">
                <template #default="scope">
                    <div  class="symbol symbol-50px me-3">
                        <img
                            :src=" scope.row.file?.url || '/media/misc/image.png'"
                            class=""
                            alt=""
                        />
                    </div>
            ...
            <el-table-column prop="price" label="Price"  class-name="text-end">
                <template #default="scope">
                    {{
                        null === scope.row.price
                            ? ""
                            : currencyFormat(scope.row.price)
                    }}
                </template>
            </el-table-column>
            <el-table-column prop="income.name" label="Income" />

            <el-table-column prop="purchase_description" label="Purchase Description" />
            ...
            <el-table-column label="Actions" width="150" class-name="!static">
                <template #default="scope">
                    <div class="space-x-standard justify-end">
                        ...
                        <AppPageEdit
                            :id="scope.row.uuid"
                            :endpoint="`/api/business/products/${scope.row.uuid}`"
                            width="60%"
                            width-lg="30%"
                            method="PUT"
                        >
