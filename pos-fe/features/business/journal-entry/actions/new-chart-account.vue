<template>
    ...
    >
        <span>Create a new chart of account</span>
        <KTIcon icon-name="plus-square" icon-class="fs-2x" />
    </div>
    <Teleport to="body">
        <Drawer
            id="add-new-chart_account"
            title="New Chart of Account"
            description="Simplest and easy way to add a new chart of account"
...
const execute = () =>
    useClient("/api/accounting/chart-accounts", {
        method: "POST",
        body: form.value,
    });
...
onMounted(() => {
    drawer.value = $drawer(`add-new-chart_account`);
});
</script>
