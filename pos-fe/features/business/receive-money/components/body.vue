<template>
    <div class="table-responsive">
        <div class="bg-white">
            <table
                class="table table-bordered table-hover table-row-gray-300 border table-rounded gy-4 gs-4"
            >
                <thead>
                    <tr
                ...
                <tbody>
                    <tr v-for="(item, index) in data!.items">
                        ...
                        <td
                            class="text-start align-middle cursor-pointer"
                            @click="productActive(index)"
                        >
                            <div
                                class="flex flex-col justify-baseline items-start gap-y-2"
                            >
                                <Select
...
const total = (key: string, items?: OfficialReceiptItem[]): number =>
...
const refill = () => {
    for (var i = (data.value?.items ?? []).length - 1; i < 3; i++) {
        data.value!.items = (
            (data.value!.items ?? []) as unknown as OfficialReceiptItem[]
        ).concat({
            product: null,
            rate: 0,
            tax_rate: 0,
...
const calculateTax = (item: InvoiceItem, tax: Tax, amount: number) => {
    console.log({ tax, amount });
    item.tax_rate = calculateTaxRate(tax, amount);
};
...
onBeforeUnmount(() => {
    dismiss("add:invoice-items");
});

onMounted(() => {
    receive("add:invoice-items", (invoices: Invoice[]) => {
        data.value!.items = [];

        var items = [];
        for (var invoice of invoices ?? []) {
            for (var item of invoice?.details ?? []) {
                items.push({
                    active: true,
                    product: {
                        ...item.product,
