<template>
    ...
    <Teleport to="body">
        <Drawer
            id="add-new-product"
            title="New Product"
            description="Simplest and easy way to add a new product"
...
import Form from "~/features/products/catalogue/form.vue";
...
const execute = () =>
    useClient("/api/products/product-categories", {
        method: "POST",
        body: form.value,
    });
...
onMounted(() => {
    drawer.value = $drawer(`add-new-product`);
});
</script>
