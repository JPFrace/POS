    ...
    <div class="d-flex flex-column flex-xl-row">
                ...
                <div class="card-body pt-15">
                    <div class="d-flex flex-center flex-column mb-5">
                        <div class="symbol symbol-100px symbol-circle mb-7">
                            <img :src="details?.file?.url" alt="Company Logo"
                                class="image-input-wrapper w-125px h-125px" />
                        </div>
                        <a class="fs-3 text-gray-800 text-hover-primary fw-bold mb-1">
                            <span class="d-block text-center">{{ details?.name?.toUpperCase() }}</span>
                        </a>
                        <div class="fs-5 fw-semibold text-muted mb-6 text-center">
                            {{ details?.address }}
                        </div>
                    </div>
                    <div class="d-flex align-items-center my-4">
                        <hr class="flex-grow-1" />
                        <span class="mx-3 text-muted fw-bold">Details</span>
                        <hr class="flex-grow-1" />
                    </div>
                    <div class="fs-6">
                        <div class="fw-bold mt-5">TIN No.</div>
                        <div class="text-gray-600">{{ details?.tin_no }}</div>
                        <div class="fw-bold mt-5">Email</div>
                        <div class="text-gray-600">{{ details?.email }}</div>
                        <div class="fw-bold mt-5">Address</div>
                        <div class="text-gray-600">{{ details?.address }}</div>
                        <div class="fw-bold mt-5">Phone</div>
                        <div class="text-gray-600">{{ details?.phone }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex-lg-row-fluid ms-lg-15">
            <ul class="nav nav-custom nav-tabs nav-line-tabs nav-line-tabs-2x border-0 fs-4 fw-semibold mb-8">
                <li class="nav-item">
                    <a class="nav-link active text-active-primary pb-4" data-bs-toggle="tab" href="#profile">
                        Company Profile
                    </a>
                </li>
            </ul>
                ...
                <div class="tab-pane fade show active" id="profile">
                    <div class="card mb-5 mb-xl-8">
                        ...
                        <div class="card-body pt-15">
                            ...
                            <div class="row mb-6" v-for="field in ['name', 'tin_no', 'email', 'address', 'phone']"
                                :key="field">
...
const form = reactive<Company>(cloneDeep(props.details ?? ({} as Company)));
