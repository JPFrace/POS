...
const handleUnpost = async () => {
    if (!isPosted.value) {
        $message("info", t("invoice.validation.not_posted"));
        return;
    }

    const result = await $swal("warning", {
        html: `Are you sure you want to unpost <strong>Invoice No. ${props.row.invoice_no}?</strong>`,
    ...
    try {
        ...
        $message(
            "error",
            error?.data?.message ??
                t("invoice.error.failed_to_unpost")
        );
    }
};
</script>