        ...
        <div class="flex gap-4 items-center mr-8">
            <div class="flex flex-col gap-1 mb-4">
                ...
                <Select
                    ...
                    url="/api/business/invoice-statuses"
            ...
            >
                <template #default="{ row }">
                    ...
                    <el-icon
                        ...
                        title="Posted & Paid invoices are locked"
                    >
                        <Lock />
                    </el-icon>
                </template>
            ...
            >
                <template #default="{ row }">
                    <div class="flex space-x-standard mr-2">
                        <template v-if="isPosted(row) || isPaid(row)">
                            <KTIcon
                                title="Edit"
                                ...
                                @click="
                                    $message(
                                        'warning',
                                        t('invoice.validation.cannot_edit')
                                    )
                                "
                            />
                        </template>
                        <template v-else>
                            <NuxtLink :to="`/business/invoice/${row.uuid}`">
                                <KTIcon
                                    title="Edit"
                                    icon-name="notepad-edit"
                                    icon-type="outline"
...
import PostStatus from "./components/invoice-actions/post.vue";
import UnpostStatus from "./components/invoice-actions/unpost.vue";
import CancelAction from "./components/invoice-actions/more/cancel.vue";
import DeleteAction from "./components/invoice-actions/more/delete.vue";
...
const summary = (param: SummaryMethodProps) => {
    ...
    columns.forEach((column, index) => {
        ...
        const property = column.property as keyof InvoiceItem;
