<template>
        ...
        <div class="flex gap-4 items-center mr-8">
            <div class="flex flex-col gap-1 mb-4">
                ...
                <Select
                    ...
                    url="/api/business/payment-statuses"
    ...
    <AppPageTable
        ...
        :prefix-key="tab"
        endpoint="/api/business/payments"
        ...
        <template #columns>
            ...
            >
                <template #default="{ row }">
                    <el-checkbox
                        v-if="row.status.name.toLowerCase() !== 'paid'"
                        :model-value="
                    ...
                    <el-icon
                        ...
                        title="Posted payments are locked"
                    >
                        <Lock />
                    </el-icon>
                </template>
            ...
            >
                <template #default="{ row }">
                    <div class="flex flex-col gap-y-1">
                        <span class="font-bold">{{ row.payee_name }}</span>
                        <div class="flex items-center text-sm">
                            <span
                                :class="
                                    row.payee.type_label?.toLowerCase() ===
                                    'customer'
                                        ? 'text-blue-500'
                                        : 'text-cyan-500'
                                "
                            >
                                {{ row.payee.type_label }}
                            </span>
            ...
            >
                <template #default="{ row }">
                    <NuxtLink
                        :to="`/business/make-payments/${row.uuid}`"
                        target="_blank"
                        ...
                        <template v-if="isPosted(row)">
                            <KTIcon
                                ...
                                @click="
                                    $message(
                                        'warning',
                                        t('make_payment.validation.cannot_edit')
                                    )
                                "
                            />
                        </template>
...
import PostStatus from "./components/payment-made-actions/post.vue";
import UnpostStatus from "./components/payment-made-actions/unpost.vue";
import CancelAction from "./components/payment-made-actions/more/cancel.vue";
import DeleteAction from "./components/payment-made-actions/more/delete.vue";

const { receive } = usePageEvent();
const search = ref();
const selectedRows = ref<any[]>([]);
const tab = ref("payments-made");
...
const isRowSelectable = (row: any) => {
    return row.status.name.toLowerCase() !== "paid";
};
...
interface SummaryMethodProps<T = PaymentItem> {
...
const summary = (param: SummaryMethodProps) => {
    ...
    columns.forEach((column, index) => {
        ...
        const property = column.property as keyof PaymentItem;
